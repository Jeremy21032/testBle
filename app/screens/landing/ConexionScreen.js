...
import { StyleSheet, View, FlatList, TouchableHighlight, ScrollView } from 'react-native'
import { Button, Icon } from '@rneui/base';
import Header from '../../components/Header';
import StyledText from '../../components/StyledText';
import theme from '../../theme/theme';
import { printer1, ConectarImpresora, VerificarBlu } from '../../services/ImpresoraService';
...
export const ConexionScreen =()=> {
    ...
    const { handlePrinterAddress, handlePrinterAddressDefault, printerAddressDefault, printerAddress } = useContext(ModernaContext)
    ...
    const imprimir = () => { printer1(ENCABEZADO, printer) }
    ...
    var ENCABEZADO = "^XA  ^FS^CI28^FO20,80^ASN^ADN,25,10^FDDirección:SAN GABRIEL OE7-89 Y VALDERRAMA ^XZ";
    ...
    return (
        <View style={styles.container}>
            ...
            <StyledText bold heading center>Conexiones</StyledText>
            ...
            <View style={{ backgroundColor: '#F0F0F0', height: 100, justifyContent: 'center', paddingHorizontal: 15 }}>
                <View style={{ /*backgroundColor: 'cyan',*/ flexDirection: 'row', alignItems: 'center' }}>
                    <Icon
                        type='feather'

                        name={statusImpresora ? 'check-circle' : 'x-circle'}
                        //color={status ? "green" : "gray"}
                        color={statusImpresora ? theme.colors.modernaGreen : theme.colors.modernaRed}
                    />
                    <StyledText style={{ paddingLeft: 5, color: statusImpresora ? "black" : theme.colors.modernaRed }}>{statusImpresora ? "¡Este dispositivo tiene Bluetooth!" : "Sin conexión a Bluetooth"}</StyledText>
                </View>
                ...
                <View style={{ /*backgroundColor: 'gray',*/ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Icon
                        ...
                        color={statusImpresora ? theme.colors.modernaGreen : theme.colors.modernaRed}
                    />
                    <StyledText style={{ paddingLeft: 5, color: statusImpresora ? "black" : theme.colors.modernaRed }}>{statusImpresora ? "¡Bluetooth  habilitado!" : "¡Bluetooth  inhabilitado!"}</StyledText>
                </View>
            </View>
            {/* ------------------------------ */}
            {statusImpresora ? <><View style={{ backgroundColor: '#F0F0F0', height: 100, justifyContent: 'center', paddingHorizontal: 15, marginTop: 15 }}>

                <StyledText style={{ marginBottom: 10 }}>Dispositivo predeterminado</StyledText>
                ...
                <Button
                    ...
                    onPress={imprimir}
                    buttonStyle={{ backgroundColor: theme.colors.modernaRed, width: 150 }}
                    containerStyle={{ alignItems: 'center', paddingTop: 15 }}
                />
            </> : <></>}
            <StyledText subtitle style={{ marginVertical: 10 }}>Conexión a internet: </StyledText>
            <View style={{ backgroundColor: '#F0F0F0', height: 50, justifyContent: 'center' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                    <Icon
                        name='check-circle'
                        type="feather"
                        color={internet ? theme.colors.modernaGreen : theme.colors.modernaRed}

                    />
                    <StyledText style={{ paddingLeft: 5, color: internet ? 'black' : theme.colors.modernaRed }}>{internet ? "¡Ya estas conectado!" : "Sin conexion"}</StyledText>
                </View>
            </View>
